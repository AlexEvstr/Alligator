...
public class BoardGen : MonoBehaviour
{
    ...
    public float ghostTransparency = 0f;
  ...
  void Start()
  {
        _currentLevel = PlayerPrefs.GetInt("LevelCurrent", 1);
        _levelText.text = $"LEVEL {_currentLevel}";
    imageFilename = GameApp.Instance.GetJigsawImageName(_currentLevel-1);
  ...
  void OnTileInPlace(TileMovement tm)
  {
    ...
    if (GameApp.Instance.TotalTilesInCorrectPosition == mTileGameObjects.Length)
    {
      ...
      menu.SetEnableGameCompletionPanel(true);
            _optionsController.DisableBGMusic();
            _optionsController.PlayWinSound();
                  _currentLevel++;
            PlayerPrefs.SetInt("LevelCurrent", _currentLevel);
